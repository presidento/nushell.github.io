import{_ as s,r as d,o as i,c,a as e,b as n,w as o,e as l,d as a}from"./app.873196f3.js";const r={},u=l(`<h1 id="custom-commands" tabindex="-1"><a class="header-anchor" href="#custom-commands" aria-hidden="true">#</a> Custom commands</h1><p>Nu&#39;s ability to compose long pipelines allows you a lot of control over your data and system, but it comes at the price of a lot of typing. Ideally, you&#39;d be able to save your well-crafted pipelines to use again and again.</p><p>This is where custom commands come in.</p><p>An example definition of a custom command:</p><div class="language-nushell ext-nushell"><pre class="language-nushell"><code>def greet [name] {
  [&#39;hello&#39; $name]
}
</code></pre></div><div class="custom-container tip"><p class="custom-container-title">TIP</p><p>The value produced by the last line of a command becomes the command&#39;s returned value. In this case, a list containing the string <code>&#39;hello&#39;</code> and <code>$name</code> is returned.</p><p>In this definition, we define the <code>greet</code> command, which takes a single parameter <code>name</code>. Following this parameter is the block that represents what will happen when the custom command runs. When called, the custom command will set the value passed for <code>name</code> as the <code>$name</code> variable, which will be available to the block.</p><p>To run the above, we can call it like we would call built-in commands:</p><div class="language-text ext-text"><pre class="language-text"><code>&gt; greet &quot;world&quot;
...
greet
</code></pre></div><p>Making a positional parameter optional does not change its name when accessed in the body. As the example above shows, it is still accessed with <code>$name</code>, despite the <code>?</code> suffix in the parameter list.</p><p>When an optional parameter is not passed, its value in the command body is equal to <code>null</code>. We can use this to act on the case where a parameter was not passed:</p><div class="language-nushell ext-nushell"><pre class="language-nushell"><code>def greet [name?: string] {
  if ($name == null) {
...
</code></pre></div>`,71),p={class:"custom-container tip"},h=l(`<p class="custom-container-title">TIP</p><p>Each line of a command has its resulting value printed out when run, as long as it isn&#39;t <code>null</code>. Hence, <code>&quot;hello all:&quot;</code> above will be printed out despite not being the return value. To prevent this, you can place <code>null</code> (or the <code>ignore</code> command) at the end of the pipeline, like so: <code>&quot;hello all:&quot; | null</code>. Also note that most file system commands, such as <code>save</code> or <code>cd</code>, always output <code>null</code>.</p><p>We could call the above definition of the <code>greet</code> command with any number of arguments, including none at all. All of the arguments are collected into <code>$name</code> as a list.</p><p>Rest parameters can be used together with positional parameters:</p><div class="language-text ext-text"><pre class="language-text"><code>def greet [vip: string, ...name: string] {
...
</code></pre></div>`,20),m=a("Let's move "),g=e("code",null,"ls",-1),f=a(" into a command that we've written:"),v=e("div",{class:"language-nushell ext-nushell"},[e("pre",{class:"language-nushell"},[e("code",null,`def my-ls [] { ls }
`)])],-1),x=a("We can use the output from this command just as we would "),w=e("code",null,"ls",-1),y=a("."),b=l(`<div class="language-text ext-text"><pre class="language-text"><code>&gt; my-ls | get name
\u2500\u2500\u2500\u252C\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
 0 \u2502 myscript.nu
 1 \u2502 myscript2.nu
...
</code></pre></div><h2 id="persisting" tabindex="-1"><a class="header-anchor" href="#persisting" aria-hidden="true">#</a> Persisting</h2>`,11),$=a("For information about how to persist custom commands so that they're visible when you start up Nushell, see the "),_=a("configuration chapter"),q=a(" and add your startup script.");function k(T,I){const t=d("RouterLink");return i(),c("div",null,[u,e("div",p,[h,e("p",null,[m,n(t,{to:"/book/commands/ls.html"},{default:o(()=>[g]),_:1}),f]),v,e("p",null,[x,n(t,{to:"/book/commands/ls.html"},{default:o(()=>[w]),_:1}),y]),b,e("p",null,[$,n(t,{to:"/book/configuration.html"},{default:o(()=>[_]),_:1}),q])])])}const F=s(r,[["render",k],["__file","custom_commands.html.vue"]]);export{F as default};
