import{_ as c,r as i,o as d,c as h,a as o,b as t,w as r,d as e,e as n}from"./app.a66ceaba.js";const l={},p=o("h1",{id:"hooks",tabindex:"-1"},[o("a",{class:"header-anchor",href:"#hooks","aria-hidden":"true"},"#"),e(" Hooks")],-1),u=e("Hooks allow you to run a code snippet at some predefined situations. They are only available in the interactive mode ("),g={href:"https://en.wikipedia.org/wiki/Read%E2%80%93eval%E2%80%93print_loop",target:"_blank",rel:"noopener noreferrer"},f=e("REPL"),v=e("), they do not work if you run a Nushell with a script ("),m=o("code",null,"nu script.nu",-1),_=e(") or commands ("),k=o("code",null,'nu -c "print foo"',-1),x=e(") arguments."),b=n('<p>Currently, we support these types of hooks:</p><ul><li><code>pre_prompt</code> : Triggered before the prompt is drawn</li><li><code>pre_execution</code> : Triggered before the line input starts executing</li><li><code>env_change</code> : Triggered when an environment variable changes</li><li><code>display_output</code> : A block that the output is passed to (experimental).</li></ul><p>To make it clearer, we can break down Nushell&#39;s execution cycle. The steps to evaluate one line in the REPL mode are as follows:</p><ol><li>Check for <code>pre_prompt</code> hooks and run them</li><li>Check for <code>env_change</code> hooks and run them</li><li>Display prompt and wait for user input</li><li>After user typed something and pressed &quot;Enter&quot;: Check for <code>pre_execution</code> hooks and run them</li><li>Parse and evaluate user input</li><li>If <code>display_output</code> is defined, use it to print command output</li><li>Return to 1.</li></ol><h2 id="basic-hooks" tabindex="-1"><a class="header-anchor" href="#basic-hooks" aria-hidden="true">#</a> Basic Hooks</h2>',5),y=e("To enable hooks, define them in your "),w=e("config"),$=e(":"),q=n(`<div class="language-text ext-text"><pre class="language-text"><code>let-env config = {
    # ...other config...
...
</code></pre></div><p>Try putting the above to your config, running Nushell and moving around your filesytem. When you change a directory, the <code>PWD</code> environment variable changes and the change triggers the hook with the previous and the current values stored in <code>before</code> and <code>after</code> variables, respectively.</p><p>Instead of defining just a single hook per trigger, it is possible to define a <strong>list of hooks</strong> which will run in sequence:</p><div class="language-text ext-text"><pre class="language-text"><code>let-env config = {
    ...other config...
